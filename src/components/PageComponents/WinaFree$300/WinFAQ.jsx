...
function WinFAQ({ 
  ...
  return (
    <section className="bg-[#F9FAFB] flex flex-col gap-12 items-center py-20 w-full px-4 md:px-8">
      {/* Heading */}
      <h2 className="font-[Oswald] text-[36px] md:text-[48px] font-bold leading-[44px] md:leading-[56px] text-[#040A16] text-center uppercase">
        {beforeHighlight}
        {beforeHighlight && " "}
            ...
            >
              <AccordionTrigger className="hover:no-underline [&>svg]:hidden group">
                <div className="flex gap-6 items-start justify-between w-full">
                  <p
                    className={`font-[Manrope] text-lg md:text-[20px] font-bold leading-[140%] text-left flex-1 group-data-[state=open]:text-[#2463D8] ${
              ...
              {faq.answer && (
                <AccordionContent className="pt-4 pb-0">
                  <p className="font-[Manrope] text-[16px] font-normal leading-[140%] text-[#1F2937]">
                    {faq.answer}
                  </p>
                </AccordionContent>
              )}
